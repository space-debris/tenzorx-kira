...
export default function Portfolio() {
  ...
  const states = useMemo(() => [...new Set((data?.loans || []).map((loan) => loan.state).filter(Boolean))], [data?.loans]);
  const districts = useMemo(() => [...new Set((data?.loans || []).map((loan) => loan.district).filter(Boolean))], [data?.loans]);
    ...
    <div className="animate-fade-in max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
        <div className="grid lg:grid-cols-5 gap-3">
          ...
          />
          <select value={filters.state} onChange={(event) => setFilters((current) => ({ ...current, state: event.target.value }))} className="border border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white">
            <option value="">All states</option>
            {states.map((state) => <option key={state} value={state}>{state}</option>)}
          </select>
          <select value={filters.district} onChange={(event) => setFilters((current) => ({ ...current, district: event.target.value }))} className="border border-slate-300 rounded-lg px-3 py-2.5 text-sm bg-white">
